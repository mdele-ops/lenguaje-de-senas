"""Construye una mascara de pixeles PRECISA (rasterizando los triangulos UV
reales) para la region de mano+antebrazo derecho, evitando los huecos que
deja un enfoque basado solo en puntos-vertice."""
import struct
...
hand_keywords = ["RightHand", "RightArm", "RightForeArm"]
hand_joint_local_idx = set(i for i, nm in enumerate(joint_names) if nm and any(k in nm for k in hand_keywords))
...
for row in range(len(joints0)):
    for c in range(4):
        j = joints0[row, c]
        w = weights0[row, c]
        if w > 0.1 and j in hand_joint_local_idx:
            vert_mask[row] = True
            break
...
# Pequeño margen para cubrir el filtrado bilineal de texturas en los bordes.
canvas = cv2.dilate(canvas, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7)))

from PIL import Image

out_path = ROOT / "tools" / "hand_mask_precise.png"
