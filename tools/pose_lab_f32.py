"""F: pulgar un poco apartado; la yema toca el indice sin clip."""
import time
...
OUT_DIR = ROOT / "tools" / "screenshots" / "lab_f32"
OUT_DIR.mkdir(parents=True, exist_ok=True)
URL = "http://localhost:8123/practica.html?letra=%E2%97%8B&v=f32yema"
...
() => {
  ...
  return {
    ...
    clip: Math.min(yemaToIndex, distTip) < 0.042,
    touch: distTip >= 0.042 && distTip <= 0.072 && yemaToIndex >= 0.042
  };
}
"""


def pose(t_curl, aside, i_curl, extra):
    return {
...
CASES = {
    "00_actual": {
        ...
        "ring": {"curl": 0.0, "spread": 4},
        "pinky": {"curl": 0.0, "spread": 6},
        "extra": {T1: {"z": 30}, "mixamorig1RightHandIndex2_041": {"x": 8}},
    },
    "01_f3": pose(0.50, 0.30, 0.52, F3),
    "02_f3_t2-28": pose(0.50, 0.30, 0.52, {T1: {"z": 22, "y": -4}, T2: {"x": -28}}),
    "03_f3_i45": pose(0.50, 0.30, 0.45, F3),
    "04_f3_t40": pose(0.40, 0.30, 0.52, F3),
    "05_f3_t2-30_i48": pose(0.50, 0.30, 0.48, {T1: {"z": 22, "y": -4}, T2: {"x": -30}}),
    "06_aside38": pose(0.50, 0.38, 0.52, F3),
    "07_z30_aside35": pose(
        0.45, 0.35, 0.50, {T1: {"z": 30, "y": -8}, T2: {"x": -30}}
    ),
    "08_z26_aside32": pose(
        0.46, 0.32, 0.48, {T1: {"z": 26, "y": -6}, T2: {"x": -32}}
    ),
    "09_z24_t2-26": pose(
        0.42, 0.34, 0.46, {T1: {"z": 24, "y": -6}, T2: {"x": -26}}
    ),
    "10_z22_t2-24_i44": pose(
        0.40, 0.32, 0.44, {T1: {"z": 22, "y": -4}, T2: {"x": -24}}
    ),
}
    ...
    with sync_playwright() as p:
        ...
        scored = sorted(
            ...
            key=lambda x: (
                ...
                abs(x[1]["distTip"] - 0.052),
                1 if x[1]["thumbUp"] < 0.72 else 0,
            ),
        )
