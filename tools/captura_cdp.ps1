# Captura practica.html cuando el modelo 3D ya cargo.
param(
  [string]$Url = "http://127.0.0.1:8006/practica.html?letra=A&v=a2",
  [string]$Out = "c:\Users\KZTRDG\Documents\Lenguaje de señas\tools\screenshots\verify_a\A_mano.png",
  [int]$Port = 9229,
  [int]$WaitSec = 90
)
...
try {
  ...
  if (-not $version) { throw "CDP no arranco en el puerto $Port" }

  $ws = [System.Net.WebSockets.ClientWebSocket]::new()
  $ws.ConnectAsync([Uri]$version.webSocketDebuggerUrl, [Threading.CancellationToken]::None).GetAwaiter().GetResult()
  ...
  while ([datetime]::UtcNow -lt $deadline) {
    try {
      ...
      if ($val -eq $true) { $ready = $true; break }
    } catch { }
  ...
  # Salta la transicion de 2 s y deja la pose de catalogo fija.
  Eval-Js @"
(() => {
