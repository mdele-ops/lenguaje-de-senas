"""F: indice arriba; pulgar horizontal cruza el indice a la mitad."""
import time
...
OUT_DIR = ROOT / "tools" / "screenshots" / "lab_f9"
OUT_DIR.mkdir(parents=True, exist_ok=True)
URL = "http://localhost:8123/practica.html?letra=%E2%97%8B&v=f9"
...
def pose(extra, t_curl=0.25, aside=-0.45):
    return {
...
CASES = {
    "00_B": pose({T1: {"y": -60}}, t_curl=0.74, aside=-0.5),
    "01_Bx40": pose({T1: {"y": -60, "x": 40}}),
    "02_Bx60": pose({T1: {"y": -60, "x": 60}}),
    "03_Bx80": pose({T1: {"y": -60, "x": 80}}),
    "04_Bx-40": pose({T1: {"y": -60, "x": -40}}),
    "05_Bx-60": pose({T1: {"y": -60, "x": -60}}),
    "06_Bz50": pose({T1: {"y": -60, "z": 50}}),
    "07_Bz70": pose({T1: {"y": -60, "z": 70}}),
    "08_Bz-50": pose({T1: {"y": -60, "z": -50}}),
    "09_By-20_z70": pose({T1: {"y": -20, "z": 70}}),
    "10_By-40_x50": pose({T1: {"y": -40, "x": 50}}),
    "11_By-40_x-50": pose({T1: {"y": -40, "x": -50}}),
    "12_x70_z40": pose({T1: {"x": 70, "z": 40, "y": -30}}),
    "13_x-70_z40": pose({T1: {"x": -70, "z": 40, "y": -30}}),
    "14_x50_y-70": pose({T1: {"x": 50, "y": -70}}),
    "15_x-50_y-70": pose({T1: {"x": -50, "y": -70}}),
}
...
def main():
    with sync_playwright() as p:
        ...
        print("best (90 + horiz + index up + close):")
        scored = sorted(
            ranked,
            key=lambda x: abs(x[1]["angle"] - 90) * 0.01
            + (1 - x[1]["thumbHoriz"])
            + max(0, 0.7 - x[1]["indexUp"])
            + x[1]["dist"] * 3,
        )
