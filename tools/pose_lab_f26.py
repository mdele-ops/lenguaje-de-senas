"""F: pulgar atras, indice al frente, hueco, pulgar vertical."""
import time
...
OUT_DIR = ROOT / "tools" / "screenshots" / "lab_f26"
OUT_DIR.mkdir(parents=True, exist_ok=True)
URL = "http://localhost:8123/practica.html?letra=%E2%97%8B&v=f26front"
...
def pose(i_curl, extra_more=None):
    extra = {T1: {"z": 30}, I2: {"z": -22}, I3: {"z": -26}}
    if extra_more:
        for name, rots in extra_more.items():
            if name in extra:
                merged = dict(extra[name])
                merged.update(rots)
                extra[name] = merged
            else:
                extra[name] = rots
...
CASES = {
    "00_f9": pose(0.62),
    "01_x-12": pose(0.62, {I2: {"z": -22, "x": -12}}),
    "02_x-20": pose(0.62, {I2: {"z": -22, "x": -20}}),
    "03_x-28": pose(0.62, {I2: {"z": -22, "x": -28}}),
    "04_i1y-16": pose(0.62, {I1: {"y": -16}}),
    "05_i58_x-20": pose(0.58, {I2: {"z": -22, "x": -20}}),
}
...
def main():
    with sync_playwright() as p:
        ...
        for name, data in CASES.items():
            ...
            print(
                f"{name}: idx={layer} shaft={m['distShaft']:.4f} tip={m['distTip']:.4f} "
        ...
        print("best index-front + gap + vertical:")
        scored = sorted(
            ranked,
            key=lambda x: (
        ...
        for name, m in scored:
            ...
            fused = "FUSED" if m["fused"] else "gap"
            print(
                f"  {name}: idx={layer} shaft={m['distShaft']:.4f} {fused} "
